...
||パスワード|input|type="password"<br>name="password"<br>minlength="8"<br>maxlength="16"<br>required|本人確認のため入力|
||退会確認チェックボックス|input|type="checkbox"<br>name="confirm"<br>required|退会に同意することを確認|
...
||ドロワー|`<div>`|id="drawer"|文章本文を表示|
||結果なしメッセージ|`<div>`|id="no\_result"|検索結果が0件の場合に「該当する内容がありません」を表示|
|マイページ|ニックネーム|`<p>`|class="user_name"|{{user.user_name}}を表示|
...
||ドロワー|`<div>`|id="drawer"|いいね・作成した文章の本文をクリックで表示(共通)|
